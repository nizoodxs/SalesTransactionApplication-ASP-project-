...
namespace SalesTransactionApplication.Controllers
{
    public class InvoicesController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "InvoiceId,InvoiceNumber,CustomerId,InoiceDate,InvoiceTotal")] Invoice invoice)
        {
            if (ModelState.IsValid)
            {
                List<SalesTransaction> salesTransaction = db.SalesTransactions.Where(x => x.CustomerId == invoice.CustomerId).ToList();
                foreach(SalesTransaction s in salesTransaction)
                {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "InvoiceId,InvoiceNumber,CustomerId,InoiceDate,InvoiceTotal")] Invoice invoice)
        {
            if (ModelState.IsValid)
            {
